...
const store = new Store<{ appData: object; settings: Settings }>({
  defaults: {
    appData: { columns: [{ id: INBOX_COLUMN_ID, name: "Inbox" }], tasks: [] },
    settings: {},
  },
});
...
function isDue(s: ScheduledTask, now: Date): boolean {
  ...
  if (r.kind === "every_n_days") {
    ...
    return (
      Math.floor(
        (now.getTime() - new Date(s.lastFiredAt).getTime()) / 86_400_000,
      ) >= r.n
    );
  }
...
function agentCompleteTask(
  ...
  branchName: string,
  prUrl: string,
): { success: boolean; reviewTaskId?: string; error?: string } {
  const data = store.get("appData") as AppData;
  const task = data.tasks.find((t) => t.id === taskId);
  if (!task) return { success: false, error: "Task not found" };
  if (task.agentGenerated)
    return {
      success: false,
      error: "Agent-generated tasks can only be completed by the user",
    };
  if (task.status !== "agent")
    return {
      success: false,
      error:
        "Task is not in agent status. The agent must pick up the task before completing it.",
    };
  ...
  const firstInColumn = updatedTasks.findIndex(
    (t) => t.columnId === task.columnId && !t.completed,
  );
  if (firstInColumn === -1) {
    ...
    if (method === "GET" && url === "/tasks") {
      ...
      const tasks = data.tasks
        ...
        .map((t) => ({
          ...t,
          columnName:
            data.columns.find((c) => c.id === t.columnId)?.name ?? null,
        }));
    ...
    if (method === "POST" && url === "/task/agent-complete") {
      try {
        ...
        if (!body?.taskId || !body?.branchName || !body?.prUrl) {
          return send(res, 400, {
            error: "taskId, branchName, and prUrl are required",
          });
        }
        const result = agentCompleteTask(
          body.taskId,
          body.branchName,
          body.prUrl,
        );
...
app.whenReady().then(() => {
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      { baseUrl, email, token }: { baseUrl: string; email: string; token: string },
    ) => {
      try {
        const projects = await fetchJiraProjects(baseUrl, email, token);
        console.log("[jira] fetched projects:", projects);
        return { projects };
      } catch (err) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        const statuses = await fetchJiraStatuses(baseUrl, email, token, projectKey);
        console.log("[jira] fetched statuses:", statuses);
        return { statuses };
      } catch (err) {
        return { error: String(err) };
      }
  ...
  ipcMain.handle(
    ...
    (
      _event,
      {
        ...
        branchName,
        prUrl,
      }: { taskId: string; branchName: string; prUrl: string },
    ) => {
  ...
  ipcMain.handle(
    "agent:input",
    (_event, { taskId, text }: { taskId: string; text: string }) => {
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      {
        ...
        taskDescription,
        repoPath,
      }: { taskId: string; taskDescription: string; repoPath: string },
    ) => {
      ...
      try {
        execSync(`git worktree add "${worktreePath}" -b "task/${taskId}"`, {
          cwd: repoPath,
        });
        ...
        store.set("appData", {
          ...
          tasks: d.tasks.map((t) =>
            t.id === taskId
              ? { ...t, status: "available", agentId: undefined }
              : t,
          ),
        });
        notifyRenderer();
      ...
      claudeProcess.on("exit", (code) => {
        console.log(
          `[agent:spawn] claude exited taskId=${taskId} code=${code}`,
        );
